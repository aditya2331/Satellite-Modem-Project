...
c = galois_table_mat_mul(G, message, q); 
c_without_noise = c;
...
c = galois_table_mat_add(c,noise,q);
...
for l_ind_5 = 1:size(err_vectors,1)
    ...
    syndrome = galois_table_mat_mul(H, e, q);
...
corrected_c = galois_table_mat_add(c,er,q); % Addition and Subtraction are the same for GF(2)
