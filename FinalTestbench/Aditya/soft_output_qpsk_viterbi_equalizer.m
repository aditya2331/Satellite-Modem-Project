function [decoded_symbols] = soft_output_qpsk_viterbi_equalizer(received_signal_downsampled, packet_size, channel_taps, known_modulated_preamble, traceback_length)
    % QPSK Viterbi Equalizer with limited trellis storage
    % Inputs:
    ...
    scale_factor = (conj(channel_taps(1))/abs(channel_taps(1)));
    ...
    truncated_preamble = known_modulated_preamble(end:-1:end - L + 2);

    % Initialize starting state
    inistate = 0;
    % for i = length(truncated_preamble):-1:1
    %     inistate = inistate + truncated_preamble(i)*4^(i-1);
    % end
    
    % State transition matrix for QPSK symbol transitions
    ...
    coded_data_packet_size = packet_size - length(known_modulated_preamble);
    ...
    for j = 1:length(received_signal_downsampled)/coded_data_packet_size
        ...
        history_for_soft_output = zeros(1, L - 1);
        ...
        for n = 1:coded_data_packet_size
            ...
            for current_state = 0:num_states-1
                % Extract QPSK symbols from state
                ...
                % if n < L     
                %     state_qpsk(n:L-1) = truncated_preamble(n:L-1);
                % end
            ...
            if n >= traceback_length
                ...
                % preload history with preamble for first traceback
                if n == traceback_length
                    history_for_soft_output(1 : L - 1) = truncated_preamble(1 : L - 1);
                end
                ...
                for k = 0:traceback_length-1
                    ...
                    if k == traceback_length-1
                        ...
                        decoded_packet(n-traceback_length+1) = scale_factor*(received_packet(n-k) - sum(history_for_soft_output.*channel_taps(2:end)));
                        % decoded_idx = (current_state - mod(current_state, 4^(L-2)))/4^(L-2);
                        history_for_soft_output = [decoded_packet(n-traceback_length+1) history_for_soft_output(1:end-1)];
                    end
        ...
        for k = 0:traceback_length-2
            ...
            % decoded_idx = (current_state - mod(current_state, 4^(L-2)))/4^(L-2);
            final_symbols(traceback_length-1-k) = scale_factor*(received_packet(n-k) - sum(history_for_soft_output.*channel_taps(2:end)));
            history_for_soft_output = [final_symbols(traceback_length-1-k) history_for_soft_output(1:end-1)];
            current_state = survivors(current_state+1, idx);
        end
